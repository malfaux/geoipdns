...
int datachk(void)
{
	...
	if (msglen != EVBUF_LEN) {
        ...
        if (read(evfd,&evbuf,msglen) != msglen) {
            dbger("truncated evbuf read!\n");
            return 0; //should never test positive
        }
    for (i = 0; i < msglen/EVBUF_LEN; i++) {
        dbger("READ EVENT %d mask=%lu\n", i, evbuf[i].mask);
        if (evbuf[i].mask & IN_Q_OVERFLOW) {
            dbger("IN_Q_OVERFLOW,");
            return 0;
        }
        if (evbuf[i].mask & IN_OPEN) dbger("IN_OPEN,");
        if (evbuf[i].mask & IN_DELETE_SELF)  dbger("IN_DELETE_SELF,");
        if (evbuf[i].mask & IN_ACCESS)  dbger("IN_ACCESS,");
        if (evbuf[i].mask & IN_ATTRIB)  dbger("IN_ATTRIB,");
        if (evbuf[i].mask & IN_CLOSE_WRITE)  dbger("IN_CLOSE_WRITE");
        if (evbuf[i].mask & IN_CLOSE_NOWRITE)  dbger("IN_CLOSE_NOWRITE");
        if (evbuf[i].mask & IN_CREATE)  dbger("IN_CREATE");
        if (evbuf[i].mask & IN_DELETE)  dbger("IN_dELETE");
        if (evbuf[i].mask & IN_DELETE_SELF)  dbger("IN_dELETE_SELF");
        if (evbuf[i].mask & IN_MODIFY)  dbger("IN_MODIFY");
        if (evbuf[i].mask & IN_MOVE_SELF)  dbger("IN_MOVE_SELF");
        if (evbuf[i].mask & IN_MOVED_FROM)  dbger("IN_MOVED_FROM");
        if (evbuf[i].mask & IN_MOVED_TO)  dbger("IN_MOVED_TO");
        if (evbuf[i].mask & IN_OPEN)  dbger("IN_OPEN");
        if (evbuf[i].mask & IN_IGNORED)  dbger("IN_IGNORED");
  ...
  dbger("datachk(): evfd=%d,cfd=%d\n",evfd,cfd);
	rc = kevent(evfd,NULL,0,&evbuf[0],1,&nowait);
	if (rc < 0)  {
	...
	EV_SET(&evbuf[0],cfd,EVFILT_VNODE,EV_ADD|EV_ONESHOT,EV_MASK,0,NULL);
	if (kevent(evfd,&evbuf[0],1,NULL,0,(struct timespec *)0) < 0) return 0;
#endif
	return 1;
}
...
#endif
extern int makeit_wildcard(char **, const char *);
...
*/
	...
	if (r == -1) return 0;
	if (r == 0) {
#ifdef WILDCARD_LOCMAP
		char *q_rewrite = alloc(255);
		if (!makeit_wildcard(&q_rewrite,q)) return doit(q,qtype);
		l = dns_domain_length(q_rewrite);
		dbger("try wildcard lookup for qlen=%d %s ", l,fqdn_read(out,q_rewrite));
		byte_copy(z+4,l,q_rewrite);
		alloc_free(q_rewrite);
		r = cdb_find(&c,z,l+4);
		if (r == -1) return 0;
		if (r == 0) return doit(q,qtype);
#else
		//clientloc = 0U;
		return doit(q,qtype);
#endif
